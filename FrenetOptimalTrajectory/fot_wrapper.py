...
def run_fot(initial_conditions, hyperparameters):
    ...
    speeds = np.empty(MAX_PATH_LENGTH)
    ix = np.empty(MAX_PATH_LENGTH)
    iy = np.empty(MAX_PATH_LENGTH)
    y_path = np.empty(MAX_PATH_LENGTH)
    iyaw = np.empty(MAX_PATH_LENGTH)
    d = np.empty(MAX_PATH_LENGTH)
    s = np.empty(MAX_PATH_LENGTH)
    speeds_x = np.empty(MAX_PATH_LENGTH)
    speeds_y = np.empty(MAX_PATH_LENGTH)
    #obs_path = []
    
    for i in range(MAX_PATH_LENGTH):
        x_path[i] = fot_rv.x_path[i]
        y_path[i] = fot_rv.y_path[i]
        speeds[i] = fot_rv.speeds[i]
        ix[i] = fot_rv.ix[i]
        iy[i] = fot_rv.iy[i]
        iyaw[i] = fot_rv.iyaw[i]
        d[i] = fot_rv.d[i]
        s[i] = fot_rv.s[i]
        speeds_x[i] = fot_rv.speeds_x[i]
        speeds_y[i] = fot_rv.speeds_y[i]
